defmodule Selecto.Builder.Subselect do
  ...
  defp build_filter_conditions(filters, target_alias) do
    build_filter_group(filters, :and, target_alias)
  end

  defp build_filter_group([], _conjunction, _target_alias), do: {[], []}

  defp build_filter_group(filters, conjunction, target_alias) when is_list(filters) do
    {clauses, params} =
      filters
      |> Enum.map(&build_filter_clause(&1, target_alias))
      |> Enum.unzip()

    joiner = if conjunction == :or, do: " OR ", else: " AND "
    wrapped_clauses = Enum.map(clauses, fn clause -> ["(", clause, ")"] end)

    case wrapped_clauses do
      [] ->
        {[], []}

      [single] ->
        {single, List.flatten(params)}

      many ->
        {["(", Enum.intersperse(many, joiner), ")"], List.flatten(params)}
    end
  end

  defp build_filter_clause({:and, filters}, target_alias) when is_list(filters) do
    build_filter_group(filters, :and, target_alias)
  end

  defp build_filter_clause({:or, filters}, target_alias) when is_list(filters) do
    build_filter_group(filters, :or, target_alias)
  end

  defp build_filter_clause({:not, filter}, target_alias) do
    {clause, params} = build_filter_clause(filter, target_alias)
    {["NOT (", clause, ")"], params}
  end

  defp build_filter_clause({:raw_sql_filter, filter_iodata}, target_alias) do
    rewritten = rewrite_root_alias(filter_iodata, target_alias)
    {rewritten, extract_params(rewritten)}
  end

  defp build_filter_clause({{:raw_sql, sql_expr}, value}, target_alias) do
    selector = rewrite_root_alias(sql_expr, target_alias)
    build_field_predicate(selector, value)
  end

  defp build_filter_clause({:array_overlap, field, values}, target_alias) do
    selector = build_filter_selector(target_alias, field)
    {[selector, " && ", {:param, values}], [values]}
  end

  defp build_filter_clause({:array_contains, field, values}, target_alias) do
    selector = build_filter_selector(target_alias, field)
    {[selector, " @> ", {:param, values}], [values]}
  end

  defp build_filter_clause({:array_contained, field, values}, target_alias) do
    selector = build_filter_selector(target_alias, field)
    {[selector, " <@ ", {:param, values}], [values]}
  end

  defp build_filter_clause({:array_eq, field, values}, target_alias) do
    selector = build_filter_selector(target_alias, field)
    {[selector, " = ", {:param, values}], [values]}
  end

  defp build_filter_clause({field, value}, target_alias) do
    selector = build_filter_selector(target_alias, field)
    build_field_predicate(selector, value)
  end

  defp build_field_predicate(selector, nil), do: {[selector, " IS NULL"], []}
  defp build_field_predicate(selector, :not_nil), do: {[selector, " IS NOT NULL"], []}
  defp build_field_predicate(selector, :not_null), do: {[selector, " IS NOT NULL"], []}

  defp build_field_predicate(selector, {:not, value}) do
    {clause, params} = build_field_predicate(selector, value)
    {["NOT (", clause, ")"], params}
  end

  defp build_field_predicate(selector, {:between, [min, max]}) do
  ...
  defp build_field_predicate(selector, {:in, values}) when is_list(values) do
    {[selector, " = ANY(", {:param, values}, ")"], [values]}
  end

  defp build_field_predicate(selector, {:not_in, values}) when is_list(values) do
    {["NOT (", selector, " = ANY(", {:param, values}, "))"], [values]}
  end

  defp build_field_predicate(selector, {:like, value}) do
    {[selector, " LIKE ", {:param, value}], [value]}
  end

  defp build_field_predicate(selector, {:ilike, value}) do
    {[selector, " ILIKE ", {:param, value}], [value]}
  end

  defp build_field_predicate(selector, {:not_like, value}) do
    {[selector, " NOT LIKE ", {:param, value}], [value]}
  end

  defp build_field_predicate(selector, {:contains, value}) do
    values = List.wrap(value)
    {[selector, " @> ", {:param, values}], [values]}
  end

  defp build_field_predicate(selector, {:contains_all, values}) when is_list(values) do
    {[selector, " @> ", {:param, values}], [values]}
  end

  defp build_field_predicate(selector, {comp, value}) do
    case normalize_filter_operator(comp) do
      nil -> {[selector, " = ", {:param, {comp, value}}], [{comp, value}]}
      sql_comp -> {[selector, " ", sql_comp, " ", {:param, value}], [value]}
    end
  end

  defp build_field_predicate(selector, value), do: {[selector, " = ", {:param, value}], [value]}

  defp build_filter_selector(target_alias, {:upper, field_name}) do
    ["UPPER(", target_alias, ".", escape_identifier(extract_filter_field_name(field_name)), ")"]
  end

  defp build_filter_selector(target_alias, field_name) do
    [target_alias, ".", escape_identifier(extract_filter_field_name(field_name))]
  end

  defp extract_filter_field_name(field_name) when is_binary(field_name) do
    cond do
  ...
  defp extract_filter_field_name(field_name) when is_atom(field_name),
    do: Atom.to_string(field_name)

  defp extract_filter_field_name(field_name), do: to_string(field_name)

  defp normalize_filter_operator(comp) when comp in [:=, "=", :eq, "=="], do: "="
  defp normalize_filter_operator(comp) when comp in [:!=, "!=", :ne, "<>"], do: "!="
  defp normalize_filter_operator(comp) when comp in [:>, ">", :gt], do: ">"
  defp normalize_filter_operator(comp) when comp in [:<, "<", :lt], do: "<"
  defp normalize_filter_operator(comp) when comp in [:>=, ">=", :gte], do: ">="
  defp normalize_filter_operator(comp) when comp in [:<=, "<=", :lte], do: "<="
  defp normalize_filter_operator(_comp), do: nil

  defp rewrite_root_alias({:param, _value} = marker, _target_alias), do: marker

  defp rewrite_root_alias(iodata, target_alias) when is_binary(iodata) do
    String.replace(iodata, "selecto_root.", "#{target_alias}.")
  end

  defp rewrite_root_alias(iodata, target_alias) when is_list(iodata) do
    Enum.map(iodata, &rewrite_root_alias(&1, target_alias))
  end

  defp rewrite_root_alias(iodata, _target_alias), do: iodata
